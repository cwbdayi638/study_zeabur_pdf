...
#include <string.h>
#include <time.h>
#include <sys/time.h>   /* gettimeofday replaces deprecated ftime */
#include <earthworm.h>
#include <kom.h>
#include <transport.h>
#include <lockfile.h>
...
int main( int argc, char **argv )
{
   ...
   int lockfile_fd;
	...
	int i,j,k,jj;
	double dis_2sta;	
	struct timeval tv;  /* replaces deprecated struct timeb/ftime */
	char tmp[120];
 ...
 *******************************************/
 ...
 if(fp==NULL)
 {
	...
	fclose(fp);
	logit("o", "tcpd: num_eew_status not found, creating new file.\n");
 }
 else
 {
	fgets(tmp,99,fp);
	...
	{	
		...
		fprintf(fp,"%d", num_eew);
		fclose(fp);		
		logit("e", "tcpd: num_eew_status corrupted, resetting to 1.\n");
	}
	else
	{
		num_eew = atoi(tmp);
 ...
 ********************************/
   ...
   logit("o", "tcpd: P-wave Velocity Model: Term_num=%d Boundary=%.1f Sw_V=%.3f Sw_VG=%.4f Dp_V=%.3f Dp_VG=%.4f\n",
          Term_num, Boundary_P, SwP_V, SwP_VG, DpP_V, DpP_VG);
   logit("o", "tcpd: S-wave Velocity Model: Term_num=%d Boundary=%.1f Sw_V=%.3f Sw_VG=%.4f Dp_V=%.3f Dp_VG=%.4f\n",
          Term_num, Boundary_S, SwS_V, SwS_VG, DpS_V, DpS_VG);
//--------------------------------------------------------------------------------  
 ...
 ************************************************/
   ...
   if(type_error==-2)
   {
	logit("e", "tcpd: Please define Message Type: TYPE_EEW in Earthworm.d.\n");
	return -1;
   }
   else if(type_error==-3)
   {
	logit("e", "tcpd: Please define Message Type: Type_EEW_record in Earthworm.d.\n");
	return -1;
   }   
 ...
 *******************************************/
   ...
   logit( "", "tcpd: Attached to public memory region %s: %ld\n", 
          RingName, RingKey );
...
/*----------------------- setup done; start main loop -------------------------*/
     ...
     while(1)
    {
      ...
      **************************/
         ...
         *********************/
           ...
           if( reclogo.type == TypeEEW ) 
           {
        		...
        		if (num_split!=14) 
        		{
        			...
        			continue;  /* was: return 0 — BUG FIX: should not exit main loop */
        		}		   
		    ...
		    { 	
				...
				{
					max_sta=vsn_trigger; 
					logit("o", "tcpd: Running processTrigger, vsn_trigger=%d\n", vsn_trigger);
		...
		{
		    logit("o", "tcpd: Listening for new event (EEW #%d)...\n", num_eew);
		    count_max=0;		
			pre_G_n = -1;
			pre_G_sta_num=-1;
			pre_proc_time = -1;	
 ...
 ***************************/
  ...
  /*3-2*/     else if( k_its("DpS_VG") ) {
        ...
        /* Deprecated parameters: accepted for backward compatibility with production tcpd.d */
            else if( k_its("Intensity_thr") ) { k_val(); logit("o", "tcpd: Intensity_thr is deprecated, ignored.\n"); }
            else if( k_its("Pv_thr") )        { k_val(); logit("o", "tcpd: Pv_thr is deprecated, ignored.\n"); }
            else if( k_its("Max_Epi_dis") )   { k_val(); logit("o", "tcpd: Max_Epi_dis is deprecated, ignored.\n"); }
/*  */    else if (k_its ("MagReject")) { 
  ...
  /*4*/     else if( k_its("GetEventsFrom") ) {
                ...
                logit("o", "tcpd: Config: nLogo=%d type=%d mod=%d instid=%d\n",
                       nLogo, GetLogo[0].type, GetLogo[0].mod, GetLogo[0].instid);
                logit("o", "tcpd: Config: MagMin=%.1f MagMax=%.1f LogSwitch=%d\n",
                       MagMin, MagMax, LogSwitch);
                logit("o", "tcpd: Config: Ignore_weight_P=%d Ignore_weight_S=%d\n",
                       Ignore_weight_P, Ignore_weight_S);
                logit("o", "tcpd: Config: Trig_dis_win=%.1f Trig_tm_win=%.1f Active_parr_win=%.1f\n",
                       Trig_dis_win, Trig_tm_win, Active_parr_win);
		...
		// printf("------------ aaa2 \n");   
      	...
      	{            
			...
			if(hyp.averr <= 0.8 && hyp.averr >= 0.1 && hyp.avwei >= 0.3 )	break;
				...
				if(QQ < 0)
				{
        			logit("e", "tcpd: processTrigger ERROR - QQ < 0, cannot remove outlier.\n");
        			return;
        		}
				...
				if(n_trigger < 5 )
				{
					logit("o", "tcpd: No report - n_trigger too low after outlier removal. averr=%.2f gap=%.0f Q=%d\n",
...
//-------------------------------------------------------------------------------------------
	...
	if(G_sta_num < 5)
	{
		logit("o", "tcpd: No report - G_sta_num=%d < 5 (n_trigger=%d).\n", G_sta_num, n_trigger);
		return;
	}
	//------------------------------------- co-site number
	if(G_sta_num <= pre_G_sta_num)
	{
		logit("o", "tcpd: No report - co-site sta count not improved (%d <= %d).\n", G_sta_num, pre_G_sta_num);
		return;
	}		
	
    logit("o", "tcpd: Running Magnitude(), n_trigger=%d\n", n_trigger);
	Magnitude(new_ntri,n_trigger, hyp, &mag, &pPgv, &ntc);

		if( mag.xMpd > MagMax || mag.xMpd < MagMin )
		{
      			logit("o", "tcpd: No report - mag.xMpd=%.2f out of range [%.1f,%.1f].\n", mag.xMpd, MagMin, MagMax);
        		return; 
        }          
...
{
      int nntc=0, i, k, dd=0, tt=0, bb=0;  /* removed unused: j, l, vv */      
      double dis, ddis, z, sum_wei;
	  /* int s_num removed: unused */
  ...
  /* removed unused assignment: j=0; */
  /* removed unused assignment: l=0; */
      nntc=0; 
  /* removed unused assignment: vv=0; */
      dd=0;
      tt=0;
...
{
    int k;
    /* time_t tp removed: unused after p=gmtime removal */

    /* struct tm *p removed: unused (p=gmtime calls discarded) */
    double t_now;	
    ...
    int ccount;
    /* int cc removed: unused */
    
    struct timeval tvnow;  /* replaces deprecated struct timeb/ftime */
    ...
    double sec1, ddis;  /* tmp removed: unused */
    ...
    t_now  = (double)tvnow.tv_sec + (double)tvnow.tv_usec/1000000.0;
    ...
    /* int cc = ccount removed: cc unused */
    
	
    	if(ccount>Term_num)  // exceeding the Term_num's report will be removed  	
		{
			logit("o", "tcpd: No report - update count %d exceeded Term_num %d.\n", ccount, Term_num);
			return;
		}
	...
	if(Show_Report==1)	
	{
       ...
       logit("o", "tcpd: Creating report %s\n", neweventfile);
	...
	sprintf(outmsg,"%5s %s %s %s %9.5f %10.5f %9.6f %9.6f %9.6f %9.6f  %7.4f %5.0f %d %d %.1f %.2f %d %c%c%c %d"               		  
	    ...
	    //ReportEEW_record( Region_out, reclogo_out, outmsg );			  
			  ...
			  logit("o", "tcpd: EEW report created: %s\n", neweventfile);
	}		
